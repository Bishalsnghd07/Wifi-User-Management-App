...
export async function GET(req: Request) {
  try {
    // 1. Read HTTP-Only Cookie for session verification
    const cookieStore = await cookies();
    const userId = cookieStore.get("wifi_user_id")?.value;

    if (!userId) {
      return NextResponse.json(
        { success: false, error: "Unauthorized: Missing session cookie" },
        { status: 401 },
      );
    }
